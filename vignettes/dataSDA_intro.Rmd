...
subtitle: "dataSDA: DatasetsandBasicStatisticsforSymbolicDataAnalysis" 
...
## Example: convert MM and iGAP formats directly to RSDA format.

The `MM_to_RSDA` function converts MM format (paired `_min`/`_max` columns) directly to RSDA format (`symbolic_tbl` with complex-encoded intervals), without requiring the intermediate steps of `RSDA_format`, `clean_colnames`, `write_csv_table`, and `read.sym.table`.

```{r prompt = TRUE}
data(mushroom.int)
mushroom.MM <- RSDA_to_MM(mushroom.int, RSDA = FALSE)
head(mushroom.MM)
mushroom.RSDA <- MM_to_RSDA(mushroom.MM)
head(mushroom.RSDA)
class(mushroom.RSDA)
```

The `iGAP_to_RSDA` function converts iGAP format directly to RSDA format in one step.
```{r prompt = TRUE}
data(abalone.iGAP)
...
The unified `int_convert_format` function can also be used with `to = "RSDA"` for auto-detected conversions.
```{r prompt = TRUE}
data(face.iGAP)
